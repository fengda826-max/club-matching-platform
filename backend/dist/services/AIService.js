...
class AIService {
     ...
     */
    async generateMatching(preferences, clubs) {
...
${clubs.map(c => `- ID: ${c.id}, 名称: ${c.name}, 分类: ${c.category}, 描述: ${c.description}, 标签: ${c.tags}`).join('\n')}`;
        return this.provider.generateStructured(systemPrompt, userPrompt, 1000);
    }
    /**
     * Chat with AI about clubs
     */
    async chat(userMessage, conversationHistory, clubs) {
...
- 如果用户问哪个社团适合他，可以根据他的兴趣推荐`;
     ...
     */
    async chatComplete(userMessage, conversationHistory, clubs) {
        const systemPrompt = `你是社团招新智能问答助手。你可以回答关于学校社团的各种问题。
使用下面提供的社团数据库作为回答依据。如果问题不在社团相关范围内，可以礼貌拒绝回答。
...
请生成社团描述:`;
     ...
     */
    async suggestTags(name, category, description) {
...
请推荐标签:`;
        const result = await this.provider.generateStructured(systemPrompt, userPrompt, 200);
