<script setup lang="ts">
	...
	import { addTag, removeTag } from '@/shared'
	...
	const hasValidAI = computed(() => aiConfigured.value)

	const stats = computed(() => ({
	  totalClubs: clubs.value.length,
	  totalMembers: clubs.value.reduce((sum, club) => sum + (typeof club.memberCount === 'number' ? club.memberCount : parseInt(club.memberCount as string) || 0), 0),
	...
	async function checkBackendHealth() {
	  try {
	    ...
	    // If we got a response, backend is available
	    backendAvailable.value = true
	    const aiHealth = await apiClient.ai.health()
	    aiConfigured.value = aiHealth.healthy
	    // Fetch statistics from backend and refresh clubs
	...
	onMounted(() => {
	  // Check health in background - don't block UI rendering
	  checkBackendHealth()
	})

	const addTagHandler = () => {
	  if (newClub.value.tags) {
	    tagInput.value = addTag(tagInput.value, newClub.value.tags)
	  }
	}

	const removeTagHandler = (tag: string) => {
	  if (newClub.value.tags) {
	    removeTag(tag, newClub.value.tags)
	  }
	}

	const addKeyActivity = () => {
	  keyActivitiesInput.value = addTag(keyActivitiesInput.value, keyActivities.value)
	}

	const removeKeyActivity = (activity: string) => {
	  removeTag(activity, keyActivities.value)
	}

	const generateClubContent = async () => {
	  if (!hasValidBackend.value) {
	    ElMessage.warning('后端服务未启动，请先启动后端服务')
	    return
	  }

	  if (!hasValidAI.value) {
	    ElMessage.warning('AI 未配置，请检查后端 AI 配置')
	    return
	  }
	  ...
	  try {
	    ...
	    const descResponse = await apiClient.ai.generateDescription(
	      newClub.value.name,
	      newClub.value.category
	    )
	    newClub.value.description = descResponse.description

	    // Generate requirements via the same generation (backend combines it)
	    // For now, backend response only gives description, we'll let user fill requirements
  ...
  <div class="admin-page">
    ...
    <div class="api-warning" v-if="!hasValidBackend">
      <div class="warning-card">
        ...
        <h3>后端服务未连接</h3>
        <p>请启动后端服务以启用数据持久化和 AI 功能</p>
      </div>
    </div>
    <div class="api-warning" v-if="hasValidBackend && !hasValidAI">
      <div class="warning-card">
        <div class="warning-icon">⚠️</div>
        <h3>AI 功能受限</h3>
        <p>请在后端配置 AI API Key 以使用 AI 自动生成功能</p>
      </div>
    </div>
    ...
    <div v-if="showAddDialog" class="modal-overlay" @click="showAddDialog = false">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          ...
          <div class="form-group ai-section" v-if="hasValidBackend && hasValidAI">
            <div class="ai-hint">
          ...
          <div class="form-group">
            ...
            <div class="tag-input-group">
              <input
                ...
                />
              <button class="add-tag-btn" @click="addTagHandler">
                <span>+</span>
              </button>
            </div>
            <div class="tags-container">
              <span
                ...
                >
                {{ tag }}
                <span class="tag-remove" @click="removeTagHandler(tag)">✕</span>
              </span>
            </div>
          </div>
