...
describe('Chat decision workspace', () => {
  ...
  it('allows reachable rules fallback to answer with sources when AI is unconfigured', async () => {
    vi.mocked(apiClient.ai.health).mockResolvedValue({ healthy: false, provider: null })
    const wrapper = await render()
    await wrapper.get('textarea').setValue('哪个社团适合编程初学者？')
    expect(wrapper.get('button[type="submit"]').attributes('disabled')).toBeUndefined()
    expect(wrapper.get('.workspace-status').text()).toContain('规则问答可用')
    expect(wrapper.get('.workspace-status').text()).not.toContain('已连接')
    expect(wrapper.text()).not.toContain('暂时无法连接服务')
    await wrapper.get('form').trigger('submit')
    expect(useUserStore().chatHistory[0]?.content).toBe('哪个社团适合编程初学者？')
    handlers.metadata?.({ sources: [{ clubId: 7, name: '编程社' }], model: 'rules-fallback' })
    handlers.chunk?.({ text: '编程社欢迎初学者，每周活动 2 小时。' })
    finish()
    await flushPromises()
    expect(wrapper.get('article.assistant').text()).toContain('编程社欢迎初学者，每周活动 2 小时。')
    expect(wrapper.get('article.assistant [aria-label="回答资料来源"]').text()).toContain('编程社')
    expect(wrapper.text()).toContain('回答已完成')
    expect(wrapper.get('.workspace-status').text()).toContain('规则问答可用')
    expect(wrapper.get('.workspace-status').text()).not.toContain('模型待确认')
  })

  it('disables only sending when the backend is unreachable, preserves drafts/history and reconnects', async () => {
    vi.mocked(apiClient.ai.health).mockRejectedValue(new TypeError('Failed to fetch'))
