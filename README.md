...
3. 规则服务先执行硬约束，再按兴趣 40、目标 25、时间 20、技能 15 计算分数。
4. DeepSeek 只为候选结果撰写说明；程序校验结构、长度和社团 ID，并拒绝模型提供的分数。
...
- 事实约束：问答先从社团表检索至多 5 条资料，SSE 首帧返回回答来源。
...
Express 模块化单体
  ...
  └─ Model Gateway ──────── DeepSeek V4 兼容 API
```

部署目标是一台腾讯云轻量应用服务器：只开放 SSH 与 HTTP/HTTPS，Node 端口和 SQLite 文件不暴露。这个规模下，模块化单体比微服务更便宜、更容易演示和排障。

## 为什么没有上 Kubernetes、Redis、向量库和微服务

当前只有一套演示数据、单实例和低并发，增加这些组件只会扩大部署面与解释成本。以下触发条件出现后再演进：

- 多实例写入或数据量进入万级：SQLite 迁移到腾讯云数据库，并增加备份与迁移演练。
- 高频会话、分布式限流或任务异步化：引入 Redis 与队列。
- 资料从结构化社团表扩展到大量规章、PDF 和历史问答：增加文档检索与向量索引。

## 技术栈

- Vue 3、TypeScript、Vite、Pinia、Element Plus
- Node.js、Express、Zod、Prisma、SQLite
- DeepSeek V4 / OpenAI-compatible Provider、POST-SSE
- Vitest、Supertest
...
访问前端 `http://localhost:5175`，后端健康检查为 `http://localhost:3001/api/health`。没有 AI Key 时仍可浏览社团和使用结构化规则匹配；自然语言提取与问答会明确提示未配置。

DeepSeek 配置使用你账号实际提供的 V4 模型 ID：
...
不在仓库里写死易过期的云价格。PoC 月成本为“一台腾讯云轻量应用服务器当期套餐价 + 域名/证书（如需）+ DeepSeek 实际输入输出 token 费用”。面试时展示云控制台当日价格与模型账单，并用运营页 token 总量代入；无 AI Key 的规则模式模型成本为 0。
...
[固定 50 用例报告](docs/evaluation/latest.md)默认是确定性 CI 基线，不调用付费模型，也不把人工字段伪装成模型成绩。当前 runner 会拒绝把 `EVALUATION_LIVE_AI=true` 冒充 Live 评测；真实 DeepSeek 结果需要补充经过审阅的适配器并人工复核事实一致性后，才能用于简历数字。
...
- 当前资料检索是结构化字段相关性，不是文档级 RAG。
- Provider 能接 OpenAI-compatible API，但 DeepSeek V4 的具体模型 ID、价格和限额以账号控制台为准。
